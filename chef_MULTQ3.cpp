...
int getSumUtil(int *st, int ss, int se, int qs, int qe, int index,int *arr)
{    int ans=0,i;
...
int getSum(int *st, int n, int qs, int qe,int *arr)
{
    ...
    return getSumUtil(st, 0, n-1, qs, qe, 0,arr);
}
...
int main()
{
	...
	while(Q--)
    {
        ...
        if(type==0)    	
    	{
    		 updateValue(arr, st, n, left-1, right-1);
    	}
    	
    	if(type==1)
    	{
    		
    		count=getSum(st, n, left-1, right-1,arr);
    		printf("%d\n",count);
    	}
