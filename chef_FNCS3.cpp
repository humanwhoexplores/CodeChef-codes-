...
int main()
{
    int n,i,tempx,tempy,Q,left,right,cond;
    scanf("%d",&n);
    int arr[n],arr2[n];
    struct node  obj[n];
    for(i=0;i<n;i++)
    ...
    // tree nodes
     // updates the entries in the initial tree
    ...
    {   
		...
		obj[i].first=tempx;
		obj[i].second=tempy;  
    	arr2[i]=getsum(st,n,tempx-1,tempy-1);  // this stores values of various functions
    	
    	
    }
   //  it is the second tree
   int  *st2 = constructST(arr2, n);    //  tis stores the sum of functions 
   
   scanf("%d",&Q);
   while(Q--)
   {
   	
   	scanf("%d%d%d",&cond,&left,&right);
   	if(cond==2)
   	 printf("%d\n",getsum(st2,n,left-1,right-1));
   	if(cond==1)
   	{
   		updateValue(arr, st, n, left-1, right);  //  updates primary tree  // right here is the value to which it is updated
